...
- 上記の単一メッセージ内検出とは別に、直前最大5件の「読みが5モーラまたは7モーラになるメッセージ」をチャンネル(スレッドの場合はスレッド自身)ごとに保持し、その並びが5-7-5になった時点でも検出する(実装は `src/senryu/chain.py` の `ChainTracker`)。1件のメッセージが単一メッセージ内検出とこの複数メッセージ結合検出の両方をトリガーし、2件返信されることもある。なお5-7-5-7-7(短歌)の複数メッセージ結合検出は、先頭3件が必ず先に5-7-5として確定してしまうため対象外(単一メッセージ内での短歌検出には影響しない)
