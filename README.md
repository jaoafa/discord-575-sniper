...
- 形態素解析(SudachiPy)によって単語境界を求め、その境界でのみ5-7-5に分割できる部分を検出する
- メッセージ全体だけでなく、長文メッセージ中の部分文字列が5-7-5になっていれば検出する
...
- 検出に成功した川柳は `data/records.db` に記録される(検出日時・投稿者・チャンネル・
  元メッセージ ID・5-7-5 各パート・該当区間の形態素解析結果を保存)。
  Discord への返信が失敗した場合でも記録は残る。閲覧用のコマンドは現時点では未実装。
